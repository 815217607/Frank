...
class SmsService
{
    ...
    public static function send($phones, $code) {
        try {
            $snoopy = new Snoopy();
            $apiKey = env('testin_sms_apikey', '');
            $secretKey = env('testin_sms_secret_key', '');
            $templateId = env('testin_sms_templateid', '');
            ...
            foreach ($phones as $phone) {
                $params['phone'] = $phone;
                $params['ts'] = round(microtime(true) * 1000, 0);
                $str = '';
                ...
                $requestPayload['sig'] = md5($str);
                $requestPayload = json_encode($requestPayload, JSON_UNESCAPED_UNICODE);
                $snoopy->set_submit_raw();
                $snoopy->submit("http://api.sms.testin.cn/sms", $requestPayload);
 ...
 *
 * 在原版基础上做了修改：
 * fetch($p,$p2) a加入了$p2参数，设定文件路径，直接将读取的内容写入指定文件，不整个放入内存
 * 加入set_submit_raw()，此时submit($p,$p2,$p3)仅适用前两个参数，$p2即为http body的内容
 *
...
class Snoopy
{
    ...
    var $agent = "Mozilla/5.0 (compatible; MSIE 5.01; Windows NT 5.0)"; // agent we masquerade as
    ...
    /*======================================================================*\
        ...
        Input:		$URI	the location of the page to fetch
                    $writeToFile 直接写入文件，不保存结果到内存,绝对路径
        Output:		$this->results	the output text from the fetch
    \*======================================================================*/

    function fetch($URI, $writeToFile='')
    {
            ...
            case "http":
                ...
                if ($this->_connect($fp)) {
                    if ($this->_isproxy) {
                        ...
                        $this->_httprequest($URI, $fp, $URI, $this->_httpmethod, '','',$writeToFile);
                    } else {
                        $path = $URI_PARTS["path"] . ($URI_PARTS["query"] ? "?" . $URI_PARTS["query"] : "");
                        // no proxy, send only the path
                        $this->_httprequest($path, $fp, $URI, $this->_httpmethod, '','',$writeToFile);
                    }
     ...
     * post raw
     */
    function set_submit_raw() {
        $this->_submit_type = 'raw';
    }
    ...
    function _httprequest($url, $fp, $URI, $http_method, $content_type = "", $body = "", $writeToFile='')
    {
        $cookie_headers = '';
        ...
        if($writeToFile!='')
            $writeToFileHandler = fopen($writeToFile,'w');
        do {
            ...
            if($writeToFile!='')
                fwrite($writeToFileHandler,$_data);
            else
                $results .= $_data;
        } while (true);

        // gunzip
        if ($is_gzipped&&$results!='') {
